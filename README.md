...
How can we efficiently and accurately analyze a tensor when only partial observations arrive over time? In an element-level streaming setting, only a subset of entries within the newest slice of a tensor is revealed at each update, either individually or in small groups. This formulation is particularly important for tensors with multiple temporal modes, where folding a signal into higher order exposes both short-term cycles and long-term trends, and new data naturally appear as elements rather than full slices. Existing streaming tensor decomposition methods, designed for slice-level updates, become inefficient or inaccurate when applied to element-level streams.
